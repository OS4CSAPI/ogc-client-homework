...
test("GET /controlStreams/{id}/commands lists commands for a ControlStream (if linked)", async () => {
  ...
  if (!root.features?.length) {
    ...
    console.warn("[commands.spec] No command features; skipping /req/command/ref-from-controlstream.");
    return;
  }

  const withCtl = root.features.filter((f: any) => {
    const p = f.properties || {};
    return p.controlStream?.id || p.controlStreamId || (Array.isArray(p.controlStreamIds) && p.controlStreamIds.length > 0);
  });

  if (!withCtl.length) {
    // eslint-disable-next-line no-console
    console.warn("[commands.spec] No controlStream linkage present; skipping /req/command/ref-from-controlstream.");
    return;
  }
  ...
  const controlStreamId =
    ...
    (Array.isArray(props.controlStreamIds) ? props.controlStreamIds[0] : undefined);

  if (!controlStreamId) {
    // eslint-disable-next-line no-console
    console.warn("[commands.spec] Could not derive controlStreamId; skipping /req/command/ref-from-controlstream.");
    return;
  }
  ...
  let nested: any;
  try {
    nested = await maybeFetchOrLoad(`commands_controlStream_${controlStreamId}`, nestedUrl);
  } catch (e) {
    // eslint-disable-next-line no-console
    console.warn(`[commands.spec] Nested commands not available for controlStream '${controlStreamId}'; skipping assertion.`);
    return;
  }
...
test("GET /commands/{id}/status returns a Status resource (if available)", async () => {
  ...
  try {
    ...
    console.warn(`[commands.spec] No status fixture or endpoint for command '${commandId}'; skipping /req/command/status.`);
    return;
  }
...
test("GET /commands/{id}/result returns a Result resource (if available)", async () => {
  ...
  try {
    ...
    console.warn(`[commands.spec] No result fixture or endpoint for command '${commandId}'; skipping /req/command/result.`);
    return;
  }
...
test("Commands optionally expose Status and Result link references", async () => {
  const root: any = await maybeFetchOrLoad("commands", getCommandsUrl(apiRoot));
  if (!root.features?.length) {
    console.warn("[commands.spec] No commands; skipping optional linkage test.");
    return;
  }
  const first = root.features[0];
  const statusLink =
