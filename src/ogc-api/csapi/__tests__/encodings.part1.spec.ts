...
test("GET /systems returns valid SensorML-JSON encoding", async () => {
  ...
  expect(["System", "PhysicalSystem", "PhysicalComponent", "Procedure"]).toContain(data.type);
...
test("Server supports content negotiation for GeoJSON and SensorML-JSON", async () => {
  ...
  const geojsonData: any = await maybeFetchOrLoad("encodings_part1_geojson", geojsonUrl);
  const sensormlData: any = await maybeFetchOrLoad("encodings_part1_sensorml", sensormlUrl);
  ...
  expect(["System", "PhysicalSystem", "PhysicalComponent", "Procedure"]).toContain(sensormlData.type);
...
test("SensorML-JSON encoding includes proper identification and definitions", async () => {
  ...
  if (data.definition) {
    ...
    // Should be a URI
    expect(data.definition.startsWith("http://") || data.definition.startsWith("https://") || data.definition.startsWith("urn:")).toBe(true);
  }
