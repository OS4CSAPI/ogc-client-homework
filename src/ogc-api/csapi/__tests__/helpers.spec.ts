...
describe("CSAPI Helper Utilities", () => {
  ...
  test("extractParameters handles null or undefined safely", () => {
    expect(extractParameters(null as unknown as Record<string, unknown>)).toEqual([]);
    expect(extractParameters(undefined as unknown as Record<string, unknown>)).toEqual([]);
  });
});
